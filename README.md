...
投稿される画像はデフォルトで2048x2048以下になるようにリサイズされます。
投稿する画像サイズを変更する場合は以下の設定を追加してください。
また、オリジナルサイズを利用したい場合は下記値に十分に大きい値を設定してください。
...
サイズは任意の数値です。
PicasaウェブアルバムではGoogle+と連携することで、2048x2048以下の画像が容量無制限でアップロード可能となります。

また、表示時に読み込まれる画像サイズはデフォルトで幅800のものを取得するようになっています。
表示する画像サイズを変更する場合は以下の設定を追加してください。
